...
DAT=read.table("../data/H1N1seasonal/H1N1seasonal_enrichment_scores_327codons", header=TRUE, sep="\t")
...
CODON=c(89,97,129,134,161,185,186,221,222,226)
for (i in 1:nrow(DAT))
  if (DAT[i,1]==89 | DAT[i,1]==97 | DAT[i,1]==129 | DAT[i,1]==134 | DAT[i,1]==161 | DAT[i,1]==185 | DAT[i,1]==186 | DAT[i,1]==221 | DAT[i,1]==222 | DAT[i,1]==226)
  {
    row=nrow(ALLELE)+1
...
GISAID=read.table("../data/H1N1seasonal/H1N1seasonal_background_strains_10alleles", header=TRUE, sep="\t")
...
PCA <- princomp(GISAID[,4:13])
pc=c(1,2)
SCORE=PCA$scores
SCORE1=cbind(SCORE,GISAID$Passage)
SCORE=cbind(SCORE1,GISAID$Year)
colnames(SCORE)=c("PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10","PASSAGE","YEAR")
...
for (i in 1:nrow(BACKGROUND))
{
  ...
  AD[nrow(AD)+1,1:2] = c(BACKGROUND[i,12],dis)
}
...
for (i in 1:nrow(BACKGROUND))
{
  ...
  DISTANCE[nrow(DISTANCE) + 1,1:3] =   suppressWarnings(c(as.integer(BACKGROUND[i,12]),as.character(BACKGROUND[i,11]),dis))
}
