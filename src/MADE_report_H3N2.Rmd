...
<span style="font-family:Arial; font-size:0.8em;">The output of MADE consists of several sections including 1) Virus Isolate Information (input from the user), 2) Results (main output), 3) Supporting Materials: a) Synopsis, a detailed background introduction b) PCA map, which plots the input strain together with all other isolates in a PCA map, c) Predicted Vaccine Efficacy, which computes vaccine efficacy using the adaptive distance of the input strain, d) Enrichment scores, which plots enrichment scores of the alleles at the 20 egg passage related codons, e) Adaptive distances, the adaptive distances of all the strains as well as egg strains, f) Related Tool, a link to FluSurver which is a popular tool that allows to further link mutations with literature-reported and 3D-structurally relevant phenotypes and mutation statistics based on data submitted to GISAID, g) References.</span>
...
boxplot(OR ~ Codon, data = ALLELE,boxwex=.6, outline=FALSE, col="gray",ylab="Enrichment score",xlab="Codon Position",cex.lab=1,cex.axis=0.8,las=2, main="Enrichment scores across key codons")
