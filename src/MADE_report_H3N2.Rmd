...
<span style="font-family:Arial; font-size:0.8em;">This report was generated using **MADE (Measuring Adaptive Distance and vaccine Efficacy using allelic barcodes)** at Genome Institute of Singapore, A*STAR, Singapore and Institute of Zoology, Chinese Academy of Sciences, Beijing, China.</span>

<span style="font-family:Arial; font-size:0.8em;">In our companion study, we found that the influenza viruses, especially H3N2 subtype, undergo strong egg passage adaptation during egg vaccine production and the strength of passage adaptation (measured as the Adaptive Distance or AD) negatively correlates with vaccine efficacy. Based on this observation, MADE enables users to 1) examine whether input strain has been passaged in embryonated eggs. 2) predict vaccine efficacy of the input candidate vaccine virus (CVV).</span>
...
fig1l <- "The enrichment scores calculated using multiple codons will define a multidimensional space. Using principle component analysis, we projected the high dimensions down to the first two principle components (PCs). When plotting all virus strains in the database onto this PCA map (each isolate is a dot), we found that most of the virus strains in the PCA map are highly clustered and only a small proportion of strains passaged in embryonated eggs form separate clusters away from the major cluster. We thus defined the adaptive distance (AD) as the distance between the major cluster and the target strain."
...
fig3l <- "Enrichment scores (ES) measures the enrichment of specific alleles in egg passaged strains (vs background sequences). High ES indicates strong egg passage adaptation. Red dot represents the enrichment scores over specific allels in the focal strain."

fig3flu <- "Flusurver (https://flusurver.bii.a-star.edu.sg) enables to highlight phenotypically or epidemiologically interesting candidate mutations for further research."
...
***
#### <span style="font-family:Arial; font-size:1em;">Adaptive Distances</span>
...
***
### <span style="font-family:Arial; font-size:1em;">Relevant Tool</span>
<div><iframe id="tpserver" scrolling="auto" src="https://mendel2.bii.a-star.edu.sg/METHODS/flusurver/beta/index_eggAdaptation.html" style="position:relative;margin:0px;width:100%;height:750px;" frameborder="0"></iframe></div>
